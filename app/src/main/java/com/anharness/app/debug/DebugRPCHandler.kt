...
class DebugRPCHandler(private val context: Context) {
    ...
    private fun crashSummary(f: File): String = try {
        ...
        val frame = body.firstOrNull { it.trimStart().startsWith("at com.anharness") }
    ...
    private suspend fun handleShellExecute(params: JSONObject): JSONObject {
        ...
        val result = try {
            ExecutionCoordinator.execute(
                ...
                lineCallback = { rawLine ->
                    val (_, urls) = com.anharness.app.terminal.MinisUrlMarker.extract(rawLine)
        ...
        for (raw in capturedUrls) {
            com.anharness.app.terminal.MinisOpenUrlBroker.offer(raw)
        }
        return JSONObject()
    ...
    private suspend fun handleUpdateCheck(): JSONObject {
        return when (val r = com.anharness.app.data.UpdateChecker.check()) {
            is com.anharness.app.data.UpdateChecker.CheckResult.UpdateAvailable -> JSONObject()
            ...
            com.anharness.app.data.UpdateChecker.CheckResult.UpToDate ->
                JSONObject().put("status", "up_to_date")
            com.anharness.app.data.UpdateChecker.CheckResult.NoReleaseAvailable ->
                JSONObject().put("status", "no_release")
            is com.anharness.app.data.UpdateChecker.CheckResult.NoApkAsset ->
                JSONObject().put("status", "no_apk_asset").put("tag_name", r.tagName)
            com.anharness.app.data.UpdateChecker.CheckResult.Forbidden ->
                JSONObject().put("status", "forbidden")
            com.anharness.app.data.UpdateChecker.CheckResult.NetworkUnreachable ->
                JSONObject().put("status", "network_unreachable")
            is com.anharness.app.data.UpdateChecker.CheckResult.Error ->
    ...
    private suspend fun handleUpdateDownload(params: JSONObject): JSONObject {
        ...
        return when (val r = com.anharness.app.data.UpdateChecker.download(context, url)) {
            is com.anharness.app.data.UpdateChecker.DownloadResult.Success -> JSONObject()
                .put("status", "ok")
                .put("path", r.file.absolutePath)
                .put("size", r.file.length())
            is com.anharness.app.data.UpdateChecker.DownloadResult.Error -> JSONObject()
                .put("status", "error")
    ...
    private fun handleUpdateInstall(params: JSONObject): JSONObject {
        ...
        val canInstall = com.anharness.app.data.UpdateChecker.canInstall(context)
        return try {
            com.anharness.app.data.UpdateChecker.installApk(context, file)
    ...
    private fun handleWriteFile(params: JSONObject): JSONObject {
        ...
        val hostFile = PRootKernel.resolveHostPath(path) ?: run {
            val rootfsDir = com.anharness.app.sandbox.RootfsManager.getInstance(context).rootfsDir
    ...
    private fun handlePermissionsList(): JSONObject {
        ...
        for (tool in com.anharness.app.offload.OffloadPermissionManager.toolRegistry) {
            arr.put(JSONObject()
                ...
                .put("currentLevel", com.anharness.app.offload.OffloadPermissionManager.getLevel(tool.toolName).name)
    ...
    /**
     * T344: Direct invocation of [com.anharness.app.sandbox.offload.ShizukuOffloadHandler]
    ...
    private fun handleShizukuExec(params: JSONObject): JSONObject {
        ...
        val handler = com.anharness.app.sandbox.offload.ShizukuOffloadHandler(context)
        ...
        val request = com.anharness.app.sandbox.NativeOffloadRequest(
            pid = -1,
    ...
    /**
     * Direct invocation of [com.anharness.app.sandbox.offload.ModelUseOffloadHandler]
    ...
    private fun handleModelUseExec(params: JSONObject): JSONObject {
        ...
        val finalArgv: List<String> = if (params.has("input")) {
            ...
            val hostFile = com.anharness.app.sandbox.PRootKernel.resolveHostPath(linuxPath)
        ...
        val app = context.applicationContext as com.anharness.app.MinisApp
        val handler = com.anharness.app.sandbox.offload.ModelUseOffloadHandler(context, app.providerRepository)
        val request = com.anharness.app.sandbox.NativeOffloadRequest(
            pid = -1,
     ...
     * [com.anharness.app.sandbox.offload.SessionsOffloadHandler] for e2e
    ...
    private fun handleSessionsExec(params: JSONObject): JSONObject {
        ...
        val app = context.applicationContext as com.anharness.app.MinisApp
        val handler = com.anharness.app.sandbox.offload.SessionsOffloadHandler(app.chatRepository)
        val request = com.anharness.app.sandbox.NativeOffloadRequest(
            pid = -1,
        ...
        return when (sub) {
            "set" -> {
                ...
                kotlinx.coroutines.runBlocking {
                    com.anharness.app.config.ConfigBridge.performWriteBatch(
                        items = items,
            ...
            "get" -> {
                ...
                com.anharness.app.config.ConfigBridge.readField(
                    path = path,
                    filter = filter,
                    page = page,
                    pageSize = pageSize,
            ...
            "topics" -> JSONObject().apply {
                put("ok", true)
                put("topics", com.anharness.app.config.ConfigBridge.allTopics())
            }
            "topic-help" -> {
                ...
                val known = com.anharness.app.config.ConfigBridge.allTopics()
                    .let { arr -> (0 until arr.length()).any { arr.optString(it) == topic } }
                if (!known) {
                    throw RPCException(-32602, "Unknown topic '$topic' — call subcommand=topics")
                }
                val fields = com.anharness.app.config.ConfigBridge.fieldsForTopic(topic)
                JSONObject().apply {
                    put("ok", true)
                    put("topic", topic)
            ...
            "audit-list" -> {
                ...
                com.anharness.app.config.ConfigBridge.auditList(limit, scope)
            }
