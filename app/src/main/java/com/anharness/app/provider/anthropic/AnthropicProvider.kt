...
import com.anharness.app.data.model.sanitizeToolId
...
class AnthropicProvider(
    ...
    internal fun resolveSystemPrompt(userPrompt: String?): JSONArray? {
        val claudeCodePrefix = com.anharness.app.auth.ClaudeOAuthManager.ANTHROPIC_OAUTH_IDENTIFIER_PROMPT
        if (isOAuth) {
    ...
    private fun buildRequest(bodyStr: String, body: JSONObject): Request {
        ...
        builder.applyUserAgentOverride(
            customUserAgent,
            defaultUserAgent = if (isOAuth) null else com.anharness.app.provider.MinisUserAgent.DEFAULT,
        )
