...
import com.anharness.app.data.model.LLMMediaAttachment
...
import com.anharness.app.provider.thinking.ThinkingResolveContext
...
import com.anharness.app.provider.failOnSilentEmptyCompletion
...
class OpenAIProvider private constructor(
     ...
     * user-authored custom rules from [com.anharness.app.provider.thinking.ThinkingRuleResolver]'s
     ...
     * the app-level [com.anharness.app.data.FastModePrefs] toggle that Codex
    ...
    internal fun resolvedServiceTier(): String? =
        if (supportsPriorityProcessing && com.anharness.app.data.FastModePrefs.isEnabled()) {
            "priority"
        } else {
            null
        }
    ...
    private val client = OkHttpClient.Builder()
        ...
        .connectionPool(com.anharness.app.network.NetworkMonitor.sharedLLMConnectionPool)
    ...
    private fun rawStreamMessage(
        ...
        val memBefore = com.anharness.app.diagnostics.MemorySnapshot.capture()
        val serStartNs = System.nanoTime()
        val bodyStr = try {
            body.toString()
        } catch (t: Throwable) {
            com.anharness.app.diagnostics.LargeAllocProbe.report(
                "openai.body.toString", -1, "model=${model.id} messages=${messages.size}",
                memBefore, serStartNs, failure = t,
            )
            throw t
        }
        if (bodyStr.length >= com.anharness.app.diagnostics.LargeAllocProbe.NOTABLE_BYTES) {
            com.anharness.app.diagnostics.LargeAllocProbe.report(
        ...
        if (!response.isSuccessful) {
            ...
            com.anharness.app.logging.AppLogger.error(
                "OpenAIProvider",
            ...
            if (com.anharness.app.BuildConfig.DEBUG) {
                com.anharness.app.debug.LLMRequestLog.add(
                    com.anharness.app.debug.LLMRequestLog.Entry(
                        provider = "openai",
                ...
                run {
                    ...
                    if (delta != null) {
                        ...
                        if (cLen + rcLen + rLen + tcLen > 0 || delta.has("role")) {
                            com.anharness.app.logging.AppLogger.debug(
                                "OpenAIProvider",
                    ...
                    msg.role == LLMMessage.Role.USER -> {
                        ...
                        for (tr in toolResults) {
                            ...
                            if (trBytes != null && trBytes.isNotEmpty() && supportsImages) {
                                val safeBytes = com.anharness.app.provider.ImageBudget
                                    ...
                                    when (part) {
                                        ...
                                        is AgentContentPart.ImageData -> {
                                            if (supportsImages) {
                                                ...
                                                val safeBytes = com.anharness.app.provider.ImageBudget.compressUnderBudget(part.data)
    ...
    private fun responsesImageBlock(
        ...
        // T-imgsize: provider-boundary backstop.
        val safeBytes = com.anharness.app.provider.ImageBudget.compressUnderBudget(data)
    ...
    internal fun buildResponsesAPIBody(
        ...
        if (!body.has("service_tier") &&
            com.anharness.app.data.FastModePrefs.isEnabled() &&
                        ...
                        if (imgParts.isNotEmpty()) {
                            ...
                            for (part in msg.contentParts) {
                                when (part) {
                                    ...
                                    else -> com.anharness.app.logging.AppLogger.error(
                                        "OpenAIProvider",
