...
object ThinkingRuleResolver {
    ...
    fun anthropicThinkingShape(
        ...
        val adaptive = com.anharness.app.provider.anthropic.AnthropicProvider
            .modelUsesAdaptiveThinking(modelId)
        if (level.isEnabled && supportsReasoning != false) {
            if (adaptive) {
                return mapOf(
                    "effort" to com.anharness.app.provider.anthropic.AnthropicProvider
                        .thinkingEffort(level),
                )
            }
            val budget = com.anharness.app.provider.anthropic.AnthropicProvider
