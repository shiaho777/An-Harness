...
import com.anharness.app.auth.OpenAIOAuthManager
import com.anharness.app.auth.OpenRouterOAuthManager
...
fun AddProviderScreen(
    ...
    var selectedVoiceTemplate by remember {
        mutableStateOf<com.anharness.app.data.model.VoiceProviderTemplate?>(null)
    }
...
private fun ChooseProviderScreen(
    ...
    onSelectVoiceTemplate: (com.anharness.app.data.model.VoiceProviderTemplate) -> Unit = {},
) {
    SettingsScaffold(
        ...
        val templates = com.anharness.app.data.model.VoiceProviderTemplate.all
        ...
        SettingsSection(
            ...
            templates.forEachIndexed { index, template ->
                val capabilityRes = when (template.capability) {
                    com.anharness.app.data.model.VoiceProviderTemplate.Capability.TTS ->
                        R.string.add_provider_voice_capability_tts
                    com.anharness.app.data.model.VoiceProviderTemplate.Capability.ASR ->
                        R.string.add_provider_voice_capability_asr
                    com.anharness.app.data.model.VoiceProviderTemplate.Capability.BOTH ->
...
private fun ConfigureProviderScreen(
    ...
    providerRepository: ProviderRepository,
    voiceTemplate: com.anharness.app.data.model.VoiceProviderTemplate? = null,
    onBack: () -> Unit,
    onSaved: () -> Unit,
) {
                            ...
                            try {
                                when (providerType) {
                                    ProviderType.kimiCode -> {
                                        ...
                                        val key = com.anharness.app.auth.KimiOAuthManager.login(
                                    ...
                                    ProviderType.anthropic -> {
                                        val key = com.anharness.app.auth.ClaudeOAuthManager.login(context, pendingInstanceId, providerRepository)
                                        maskedToken = maskOAuthToken(key)
                                    }
                                    ProviderType.openAI -> {
                                    ...
                                    ProviderType.xAI -> {
                                        val key = com.anharness.app.auth.XAIOAuthManager.login(context, pendingInstanceId, providerRepository)
                                        maskedToken = maskOAuthToken(key)
                                    }
                                    else -> {
                                ...
                                // signal.
                                errorMessage = if (e is com.anharness.app.auth.OAuthNetworkUnreachableException) {
